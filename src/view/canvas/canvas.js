class draw {
  constructor(imgWidth = 300, imgHeight = 200, canvasWidth = 1100, canvasHeight = 800) {
    this.type = 'pen'
    this.isDraw = false
  ...
  init () {
    ...
    this.penal.addEventListener('mousemove', ({ clientX, clientY }) => {
      ...
      if (this.type === 'pen') {
        ...
        this.pen.clearRect(x - 10, y - 10, this.pen.lineWidth, this.pen.lineWidth)
        ...
        this.pen.arc(Math.abs(x - originX) + newOriginX, Math.abs(y - originY) + newOriginY, r, 0, 2 * Math.PI)
